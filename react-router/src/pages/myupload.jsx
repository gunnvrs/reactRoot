...
function Myupload() {
  ...
  const [imageRows, setImageRows] = useState(() => {
    // Retrieve image rows from local storage
    const rows = JSON.parse(localStorage.getItem("imageRows"));
    return rows ? rows : [];
  });
  ...
  const imagesQuery = query(imagesCollectionRef, where("uploadedByUid", "==", currentUser.uid));

  const uploadFile = () => {
    if (imageUpload == null) return;

    const fileName = currentUser.uid + "_" + imageUpload.name + v4();
  ...
  useEffect(() => {
    getDocs(imagesQuery, { orderBy: "createdAt" })
      .then((querySnapshot) => {
